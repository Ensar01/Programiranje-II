...
public:
    ...
    //Z1.3 Copy ctor
    Datum(const Datum& obj) // CONST ZNACI DA SE OVAJ OBJEKAT NECE MIJENJATI U OVOJ FUNKCIJI
    {
    ...
    //Z1.4 Move ctor
    Datum(Datum&& obj) //&&- indikator za move konstruktor i nema rijeci const jer moramo modifikovati ovaj objekat
    {
        _dan = obj._dan;
        obj._dan = nullptr;
        _mjesec = obj._mjesec;
...
public:
    ...
    {
        delete[]_ime; //kod nizova karaktera pozivamo prvo brisanje pa ponovo alociramo
        _ime = AlocirajIKopiraj(ime);
    }
    ...
    void SetDatumRodjenja(Datum datumRodjenja) //ovako se kreira novi objekat i kopiraju se vrijednosti objekta, a kada bi bilo Datum& onda bi se proslijedjivao direktno  cijeli objekat  sa adresom                                             
    {                                                   // Datum* salje se adresa preko koje se moze doci do cijelog objekta
        if (_datumRodjenja == nullptr)
        {
            _datumRodjenja = new Datum;   //ista je logika kad su setteri za int i koris def 
        }
    ...
    void SetSpol(bool spol) // i za bool je ista logika kao za int i datum
    {
        if (_spol == nullptr)
        {
            _spol = new bool;
...
public:
    ...
    {                                        // sa hederima ovo radimo kada imamo klasican objekat koji je atribut struct, kada bi bio pointer na _datumPremijere ne bi bilo potrebno
        _naziv = AlocirajIKopiraj(naziv);            // bilo bi _datumPremijere= new Datum (datum.GetDan(), datum.GetMjesec(), datum.GetGodina());
    ...
    //Z3.2
    Epizoda(const Epizoda& obj) : _datumPremijere(obj._datumPremijere) { //konstruktor kopije od child klase poziva konstruktor kopije od roditeljske klase
    ...
    {
        strcpy_s(_kratakSadrzaj, 100, kratakSadrzaj);//staticki niz
    }
    ...
    void ProsiriNizOcjena(int prosiriZa)
    {
        ...
        int* temp = _ocjene; //pamti stari niz
...
public:
    ...
    //Z4.1
    Uloga(Glumac& glumac, const char* opis, const char* tip)// : _glumac(new Glumac(glumac)) moglo je i sa hederom
    {
    ...
    Uloga(const Uloga& obj)
    {
        ...
        _opis = AlocirajIKopiraj(obj._opis); //mozemo koristiti i gettere a i direktno pristupati atributima 
    ...
    void SetGlumac(Glumac glumac)
    {
        if (_glumac == nullptr)
        {
            _glumac = new Glumac; // ako je kreiran pozivamo njegov dflt ctor 
        }
        // inace pozivamo settere
    ...
    ~Uloga()
    {
        delete _glumac; //dealociramo objekat
        _glumac = nullptr;
        delete[] _opis, delete[] _tipUloge;
