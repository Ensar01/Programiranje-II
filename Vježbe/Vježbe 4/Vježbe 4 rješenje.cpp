...
public:
    ...
    {
        _dan = new int(d); //ovi pojedinacni pozivi su ctor kopija jer se kopiraju vrijednosti iz varijabli u dinamicki kreirane varijable
        _mjesec = new int(m);
        _godina = new int(g);
    }
    ...
    //Z1.5 :: Operator = (assignment operator) 
    Datum& operator= (const Datum& obj) //obavlja funkcije koje su do sada obavljali setteri ali ne pojedinacno nego sve odjednom  
    {
        if (this == &obj)
        {
            return *this;
    ...
    friend ostream& operator << (ostream& COUT, const Datum obj); // Ako zelimo da globalna funkcija ima pristup clanovima odnosno elementima odredjene klase 
    friend bool operator == (const Datum& d1, const Datum& d2);                //dovoljno je da napisemo friend i dodamo identican prototip
...
//Z1.9 :: Ispisati datum
ostream& operator << (ostream& COUT, const Datum obj) //operator redirekcije, uglavnom se koristi u svrhu ispisa i to je binarni operator 
                                                            //objekti ostream klase koriste se radi komunikacije sa konzolom 
{
...
{
    int dan = obj.GetDan(), mjesec = obj.GetMjesec(), godina = obj.GetGodina(); // mogli smo proglasit i friend funkcijom
...
bool operator <(const Datum& d1, const Datum& d2)
{
    return !(d1 >= d2);   //suprotno od > je < i = zbog toga stavljamo <= jer kada bi samo stavili < dobili bi nefunkcionalan dio koda
}
...
class Clan {
    ...
    bool* _spol;
    static int _clanIdCounter; //inicijalizacija statickih varijabli pogotovo onih koje su elementi datih klasa se radi izvan
public:

    //Z2.0 :: Vratiti vrijednost statickog atributa _clanIdCounter
    static int GetCounter()
    {
        return Clan::_clanIdCounter; // ne moramo pisati Clan:: ovdje ga koristimo cisto da naglasimo da se radi o statickoj varijabli
    ...
    Clan() : _clanId(Clan::_clanIdCounter) // Kada god imamo konstantu moramo je inicijalizvoati unutar headera jer je inace ne mozemo postaviti 
    {
        Clan::_clanIdCounter++;
    ...
    Clan(const char* korisnickoIme, const char* lozinka, Datum datumReg, bool spol) :_clanId(Clan::_clanIdCounter)
    {
        Clan::_clanIdCounter++;
...
{
    return (strcmp(c1.GetKorisnickoIme(), c2.GetKorisnickoIme()) == 0) ? true : false;
    // funkcija vraca int vrijednosti i moze vratiti 1, 0 i -1 ako vrati 1 to znaci da je 1 niz veci od drugog ako vrati -1 onda je obratno a nula jednako
}

class Post {
    char* _postId;//identifikator
...
public:
    ...
    Sekcija& operator = (const Sekcija& obj)
    {
        ...
        _trenutnoPostova = obj._trenutnoPostova; //Izjednacavanje
        for (int i = 0; i < _trenutnoPostova; i++)
        {
            _postovi[i] = new Post(*obj._postovi[i]);
        }
...
public:
    ...
    Forum(const Forum& obj)
    {
        ...
        _clanovi = new Clan[obj._maxClanova];
