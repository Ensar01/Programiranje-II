...
{
private:
    shared_ptr<int> _dan; //shared pointer je pointer koji moze da dijeli vlasnistvo nad nekom varijablom nad drugim shared pointerima (smart pointer)
    shared_ptr<int> _mjesec;
    shared_ptr<int> _godina;
public:
    //Z1.1
    Datum() // nije potrebno jer su svi vec nullptr
    {
    }
    ...
    {
        _dan = make_shared<int>(dan); //make shared f-ja radi sama alokaciju prostora tako da bi new int(dan) bilo besmisleno jer bi dvaputa alocirali 
    ...
    {
        //_dan=obj._dan; imali bi dva pokazivaca koji pokazuju na isti sadrzaj 
    ...
    ~Datum()
    {
        ...
        //_dan.use_count(); govori nam koliko pokazivaca pokazuje na objekat
    }
...
public:
    ...
    DinamickaKolekcija(int maxElemenata = 100)
    {
        ...
        _elementi = new T[maxElemenata];//poziv dflt ctora za svaki element niza 
    }
    //Z2.2
    DinamickaKolekcija(const DinamickaKolekcija<T>& kolekcija)
    {
        ...
        _elementi = new T[kolekcija._maxElemenata];
    ...
    DinamickaKolekcija(DinamickaKolekcija<T>&& kolekcija)
    {
        ...
        _maxElemenata = kolekcija._maxElemenata; //posto je konstanta ne treba stavljati na nulu
    ...
    T& operator [] (int index) const //posto je & vracamo original vrijednosti, a da nema bile bi kopije 
    {
    ...
    bool UkloniZadnjiElement()
    {
        ...
        _trenutno--; //umanjimo i prepisujemo preko vec postojecih objekata 
    }
    //Z2.9
...
public:
    ...
    ~Sahista()
    {
        ...
        _spol.reset();// reset fakticki dealocira memoriju
    }
...
bool operator == (const Sahista& obj1, const Sahista& obj2)
{
    return strcmp(obj1._imePrezime, obj2._imePrezime) == 0; // funkcija pocinje uporedjivati prve karaktere od oba stringa
                                            // ako su jednaki nastavlja se dok ne dodje do razlicitih parova ili dostigne null terminirajuci karakter
}
...
{
    Sahista* najmladji = &sahisti[0]; //postavljamo vrijednost na adresu prvog elementa f-je
...
public:
    ...
    //Z5.2
    Par(const Par<T1, T2>& obj) : _lijevi(obj._lijevi),
        _desni(obj._desni)
    {
    }

    Par(Par<T1, T2>&& obj) : _lijevi(move(obj._lijevi)),
